...
export default function HomePage() {
  ...
  const downloadPDF = () => {
    ...
    doc.setFontSize(20);
    doc.text('LeetCode Wrapped', 10, 20);
  
   
    doc.setTextColor(textColor);
    doc.setFontSize(16);
    doc.setFillColor(backgroundColor);
    doc.rect(10, 40, 190, 60, 'F'); 
  
    doc.text(`Username: ${userData.username}`, 15, 50);
    doc.text(`Global Rank: ${userData.profile?.ranking}`, 15, 60);
    doc.text(`Total Solved: ${userData.submitStats?.acSubmissionNum?.[0]?.count || 0}`, 15, 70);
    doc.text(`Reputation: ${userData.profile?.reputation || 0}`, 15, 80);
    doc.text(`Total Submissions: ${userData.submitStats?.totalSubmissionNum?.[0]?.count || 0}`, 15, 90);
  
   
    doc.setFontSize(18);
    doc.setTextColor(headerColor);
    doc.text('Problem Solving Distribution:', 10, 110);
  
    doc.setFontSize(14);
    doc.setTextColor(textColor);
  
    userData.submitStats?.acSubmissionNum?.forEach((item: { difficulty: string; count: number }, index: number) => {
      const yPosition = 120 + index * 15;
      doc.text(`${item.difficulty}: ${item.count}`, 15, yPosition);
  
     
      const progressBarWidth = (item.count / userData.submitStats.acSubmissionNum[0].count) * 100;
      doc.setFillColor(headerColor);
      doc.rect(60, yPosition - 5, progressBarWidth, 10, 'F');
      doc.setFillColor(backgroundColor);
      doc.rect(60 + progressBarWidth, yPosition - 5, 100 - progressBarWidth, 10, 'F');
    });
  
    
    doc.setFontSize(12);
    doc.setTextColor('#888888');
    doc.text('Generated by Shinjan ', 10, 290);
  
     
    doc.save(`${userData.username}_LeetCode_Stats.pdf`);
  };
      ...
      <div className="relative z-10 text-white">
        <div className="max-w-7xl mx-auto px-4 py-12">
          ...
          <div className="text-center space-y-4 mb-16">
            ...
            <h1 className="text-6xl font-bold text-[#FFD700] tracking-tight">LeetCode Wrapped</h1>
            ...
            <div className="space-y-8">
              <GlassCard className="rounded-2xl p-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <StatsCard icon={Target} label="Total Solved" value={userData.submitStats?.acSubmissionNum?.[0]?.count || 0} />
                  <StatsCard icon={Award} label="Reputation" value={userData.profile?.reputation || 0} />
                  <StatsCard icon={Code2} label="Submissions" value={userData.submitStats?.totalSubmissionNum?.[0]?.count || 0} />
                </div>
              </GlassCard>
              ...
              <button
                onClick={downloadPDF}
                className="px-6 py-3 rounded-lg bg-[#FFD700] text-black font-bold hover:bg-[#FFD700]/90 transition-all"
              >
                Download PDF
              </button>
            </div>
